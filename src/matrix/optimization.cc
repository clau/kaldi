  ...
  if (k_ % opts_.L == 0) {
    ...
    if (t_ > 0) {
      ...
      if (reset_fisher_memory) {
        ...
        Vector<Real> fs(f_len);
        fs.AddMatVec(1.0, fisher_t, kNoTrans, s, 0.0);

        Vector<Real> y(dim);
        y.AddMatVec(1.0, fisher_t, kTrans, fs, 0.0);
        y.Scale(1.0 / f_len);
...
void OptimizeSsvm<Real>::EstimateInverseHessian(const VectorBase<Real> &gradient) {
  if (k_ == 0) {
    ...
    Real u = opts_.phi * (VecVec(gradient, x) / VecVec(gradient, Hg)) + (1.0 - opts_.phi) * xg / gHg;
