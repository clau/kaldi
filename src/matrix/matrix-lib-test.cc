...
template<typename Real> static void UnitTestAdaQn() {
  ...
  for (MatrixIndexT iter = 0; iter < 3; iter++) {
    ...
    OptimizeAdaQn<Real> opt_adaqn(init_x, opts);
    MatrixIndexT num_iters = 0;
    Real c = 0.01;
    Real sign = (minimize ? -1.0 : 1.0); // function has a maximum not minimum..
    while (opt_adaqn.RecentStepLength() > 1.0e-04) {
      KALDI_VLOG(2) << "Last step length is " << opt_adaqn.RecentStepLength();
      const VectorBase<Real> &x = opt_adaqn.GetProposedValue();
      ...
      if (opt_adaqn.ShouldResetFisherMemory()) {
        ...
        Real f_o = GetFunctionValue<Real>(sign, c, x_o, v, S);
        Real f_n = GetFunctionValue<Real>(sign, c, x_n, v, S);

        reset_fisher_memory = f_n > 1.01 * f_o;
      }

      opt_adaqn.DoStep(f, df_dx, reset_fisher_memory);
      num_iters++;
    }
    Vector<Real> x (opt_adaqn.GetValue());
    Vector<Real> diff(x);
    diff.AddVec(-1.0, x_opt);
    KALDI_VLOG(2) << "AdaQn finished after " << num_iters << " function evaluations.";
